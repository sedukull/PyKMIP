...
class TestKmipClientIntegration(testtools.TestCase):
    ...
    def test_symmetric_key_create_get_destroy(self):
        """
        Test that the KmipClient can create, retrieve, and destroy a symmetric
        key.
        """
        uid = self.client.create_symmetric_key(
            enums.CryptographicAlgorithm.AES, 256)
        self.assertIsInstance(uid, six.string_types)

        key = self.client.get_secret(uid)
        ...
        self.client.destroy_secret(uid)

        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.get_secret, uid)
        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.destroy_secret, uid)

    def test_symmetric_key_register_get_destroy(self):
        """
        Test that the KmipClient can register, retrieve, and destroy a
        symmetric key.
        """
        ...
        uid = self.client.register_key(key)
        self.assertIsInstance(uid, six.string_types)

        result = self.client.get_secret(uid)
    ...
    def test_asymmetric_key_pair_create_get_destroy(self):
        """
        Test that the KmipClient can create, retrieve, and destroy an
        asymmetric key pair.
        """
        public_uid, private_uid = self.client.create_asymmetric_key_pair(
        ...
        public_key = self.client.get_secret(public_uid)
        ...
        private_key = self.client.get_secret(private_uid)
        ...
        self.client.destroy_secret(public_uid)
        self.client.destroy_secret(private_uid)

        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.get_secret,
            public_uid)
        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.get_secret,
            private_uid)
        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.destroy_secret,
            public_uid)
        self.assertRaises(
            exceptions.KmipOperationFailure, self.client.destroy_secret,
            private_uid)
    ...
    def test_public_key_register_get_destroy(self):
        """
        Test that the KmipClient can register, retrieve, and destroy a public
        key.
        """
    ...
    def test_private_key_register_get_destroy(self):
        """
        Test that the KmipClient can register, retrieve, and destroy a private
        key.
        """
        # Key encoding obtained from Section 13.4 of the KMIP 1.1 test
        # documentation.
